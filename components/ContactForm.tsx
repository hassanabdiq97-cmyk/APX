...
export const ContactForm: React.FC = () => {
  ...
  return (
    <section id="kontakt" className="py-32 bg-slate-950 relative border-t border-white/5">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-6xl mx-auto glass-panel rounded-sm overflow-hidden flex flex-col md:flex-row shadow-2xl border border-white/10">
           
           {/* Left Panel: Info */}
           <div className="md:w-5/12 bg-slate-900/90 p-12 relative overflow-hidden border-r border-white/5 flex flex-col justify-between">
               ...
               <div className="relative z-10">
                   <div className="w-14 h-14 bg-slate-950 border border-white/10 rounded flex items-center justify-center mb-10 text-safety shadow-lg">
                       <Terminal size={28} strokeWidth={1.5} />
                   </div>
                   
                   <h3 className="text-3xl font-black text-white mb-8 uppercase tracking-tight leading-none">
                   ...
                   <div className="space-y-8 font-mono text-sm text-slate-400 mb-12">
                       <div className="flex items-start gap-4 group">
                          <MapPin size={18} className="text-slate-600 group-hover:text-safety transition-colors mt-1 shrink-0" />
                          <div>
                              <strong className="block text-white uppercase text-xs tracking-widest mb-1">{t.contact.address_label}</strong>
                              <span>Solothurnstrasse 44, 2543 Lengnau</span>
                          </div>
                       </div>
                       <div className="flex items-start gap-4 group">
                          <Phone size={18} className="text-slate-600 group-hover:text-safety transition-colors mt-1 shrink-0" />
                          <div>
                              <strong className="block text-white uppercase text-xs tracking-widest mb-1">{t.contact.hotline_label}</strong>
                              <a href="tel:+41321234567" className="text-white font-bold hover:text-safety transition-colors cursor-pointer">+41 32 123 45 67</a>
                          </div>
                       </div>
                       <div className="flex items-start gap-4 group">
                          <Globe size={18} className="text-slate-600 group-hover:text-safety transition-colors mt-1 shrink-0" />
                          <div>
                              <strong className="block text-white uppercase text-xs tracking-widest mb-1">{t.contact.digital_label}</strong>
               ...
               <div className="relative z-10 mt-auto bg-emerald-500/5 border border-emerald-500/10 p-4 rounded backdrop-blur-sm">
                   <div className="flex items-center gap-2 mb-2">
                       ...
                       <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">{t.contact.status}</span>
                   </div>
               </div>
           </div>

           {/* Right Panel: Form */}
           <div className="md:w-7/12 p-12 bg-slate-950/50 relative">
               ...
               {isSuccess ? (
                   <div className="h-full flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500">
                       ...
                       <h4 className="text-2xl font-black text-white uppercase mb-4">{t.contact.form.success_title}</h4>
                       <p className="text-slate-400 max-w-sm leading-relaxed mb-8">{t.contact.form.success_desc}</p>
                   ...
                   <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                       ...
                       {formData.reason === ContactReason.EMERGENCY ? (
                          <div className="bg-safety/10 border border-safety p-8 rounded-sm text-center animate-in fade-in zoom-in-95 duration-300">
                              ...
                              <h4 className="text-2xl font-black text-white uppercase mb-4 tracking-tight">{t.contact.form.emergency.title}</h4>
                              
                              <p className="text-slate-300 mb-8 text-sm font-light leading-relaxed">
                          ...
                          <>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2 group">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest group-focus-within:text-white transition-colors">{t.contact.form.name}</label>
                                    <input 
                                        type="text" name="name" required value={formData.name} onChange={handleInputChange} 
                                        className="w-full bg-slate-900 border border-white/10 p-4 text-white focus:border-safety focus:bg-slate-800 outline-none transition-all text-sm font-mono rounded-sm placeholder:text-slate-700"
                                        placeholder="Max Muster"
                                    />
                                </div>
                                <div className="space-y-2 group">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest group-focus-within:text-white transition-colors">{t.contact.form.company}</label>
                                    <input 
                            ...
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest group-focus-within:text-white transition-colors">{t.contact.form.phone}</label>
                                <input 
                            ...
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest group-focus-within:text-white transition-colors">{t.contact.form.details}</label>
                                <textarea 
                                    name="message" rows={4} value={formData.message} onChange={handleInputChange} 
                                    className="w-full bg-slate-900 border border-white/10 p-4 text-white focus:border-safety focus:bg-slate-800 outline-none transition-all text-sm font-mono resize-none rounded-sm placeholder:text-slate-700"
                                    placeholder="..."
                                ></textarea>
                            </div>
                            ...
                            <div className="pt-4">
                                ...
                                <div className="text-center text-[10px] text-slate-600 mt-4 font-mono flex items-center justify-center gap-1">
                                    <Lock size={10} />
